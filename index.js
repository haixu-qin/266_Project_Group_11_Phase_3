...
console.log('reg');
  const user2 = { username, password, balance } = req.body;
console.log(user2);
console.log(user2.user.password);
  
  const user3 = await User.findOne({ username: user2.user.username });
  if (user3) {
...
app.post('/getbal', async (req, res) =>{
  ...
  const user3 = await User.findOne({ username: t.username });
...
const result = await User.updateOne({ username: t.username }, { balance: fbal }); 
  console.log(result);
